...
<div class="bitcoin-page">
	<div class="row">
		...
		{if $nicehash_error}
		<p class="alert alert-danger">Oops! Seems like Nicehash is having some issues. Check back soon!</p>
		{else}
		<p>
			<strong>Wallet Confirmed: </strong>{$nicehash_wallet_confirmed|string_format:"%.8f"} BTC |
			<strong>Wallet Pending: </strong>{$nicehash_wallet_pending|string_format:"%.8f"} BTC |
			<strong>Mining Balance: </strong>{$nicehash_mining_balance|string_format:"%.8f"} BTC
		</p>
		...
		<div class="well">
			<p>These are the currently active workers mining on the Nicehash pool for the <a href="https://www.nicehash.com/index.jsp?p=miners&addr={$nicehash_key}" target="_blank">{$nicehash_key}</a> hash, feel free to join us!</p>
			{$workershtml}
		</div>
		{/if}
	</div>
